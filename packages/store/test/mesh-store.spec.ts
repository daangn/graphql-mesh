  ...
  describe('MeshStore', () => {
    ...
    it('should validate writes and prevent write in case of a validation error', async () => {
      ...
      try {
        ...
        expect(e.message).toBe(`Validation failed for "file.json" under "test"!`);
        expect((e as ValidationError).originalError.message).toBe(`Validation failed! you changed 1 to 2!`);
      }
